#!/usr/bin/env bash
# backup-subtitle-toolkit.sh — backs up the Subtitle Toolkit source to the
# RHLS-softdev GitHub account, mirroring the Shikibu backup pattern:
#   subtitle-toolkit  - the app source (excl. node_modules/dist/tsbuildinfo)
...
push_dir() { # $1 = repo name, $2 = source dir, $3 = commit msg
  ...
  tar --exclude=./node_modules --exclude=./dist --exclude=./tsconfig.tsbuildinfo \
      --exclude=./.env.local --exclude=./.env --exclude=./convex/_generated \
...
make_repo "subtitle-toolkit" "Subtitle Toolkit - browser-first SRT subtitle toolkit (source backup; node_modules/dist excluded)"
push_dir "subtitle-toolkit" "$STK_SRC" "backup: Subtitle Toolkit source"

echo
echo "== backup URLs =="
echo "  https://github.com/$USER/subtitle-toolkit"
